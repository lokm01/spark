...
class DataFrameFunctionsSuite extends QueryTest with SharedSQLContext {
  ...
  test("transform function") {
    val dummyFilter = (c: Column) => c.isNull || c.isNotNull
    ...
    checkAnswer(
      idf.select(transform('i, "a", _$("a") * 2)),
      Seq(
        Row(Seq(2, 4, 6, 8, 10)),
        Row(Seq.empty),
        Row(null))
    ...
    checkAnswer(
      idf.filter(dummyFilter('i)).select(transform('i, "a", _$("a") * 2)),
      Seq(
        Row(Seq(2, 4, 6, 8, 10)),
        Row(Seq.empty),
        Row(null))
    ...
    val sdf = Seq(
      Seq("a", "b", "c", "d", "e"),
      Seq(null, "b", null, "d", null),
      Seq.empty,
      null
    ).toDF("s")

    checkAnswer(
      sdf.select(transform('s, "a", concat(_$("a"), _$("a")))),
      Seq(
        Row(Seq("aa", "bb", "cc", "dd", "ee")),
        Row(Seq(null, "bb", null, "dd", null)),
        Row(Seq.empty),
        Row(null))
    )
    checkAnswer(
      sdf.filter(dummyFilter('s)).select(transform('s, "a", concat(_$("a"), _$("a")))),
      Seq(
        Row(Seq("aa", "bb", "cc", "dd", "ee")),
        Row(Seq(null, "bb", null, "dd", null)),
        Row(Seq.empty),
    ...
    checkAnswer(
      isdf.select(transform('i, "a", transform('s, "j", concat(_$("j"),
        _$("a"), _$("j"))))),
      Seq(Row(Seq(Seq("a2a", "b2b", "c2c"), Seq("a3a", "b3b", "c3c")))))

    val adf = Seq(
      Seq(null, "2", null, "4", "5", null),
      Seq(),
      null
    ).toDF("arr")

    checkAnswer(
      adf.select(transform($"arr", "a", _$("a").isNull)),
      Seq(
        Row(Seq(true, false, true, false, false, true)),
        Row(Seq.empty),
        Row(null)))
  }
