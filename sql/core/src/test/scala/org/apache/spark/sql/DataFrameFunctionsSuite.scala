...
class DataFrameFunctionsSuite extends QueryTest with SharedSQLContext {
  ...
  test("transform function") {
    ...
    checkAnswer(
      idf.filter(dummyFilter('i)).select(transform('i, _$ * 2)),
      Seq(
        Row(Seq(2, 4, 6, 8, 10)),
        Row(Seq.empty),
        Row(null))
    ...
    checkAnswer(
      sdf.select(transform('s, concat(_$, _$))),
      Seq(
    ...
    checkAnswer(
      sdf.filter(dummyFilter('s)).select(transform('s, concat(_$, _$))),
      Seq(
        Row(Seq("aa", "bb", "cc", "dd", "ee")),
        Row(Seq(null, "bb", null, "dd", null)),
        Row(Seq.empty),
